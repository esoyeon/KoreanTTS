...
def train(log_dir, config):
    ...
    with tf.variable_scope('datafeeder') as scope:
        ...
        train_feeder = DataFeederTacotron2(coord, data_dirs, hparams, config, 16,data_type='train', batch_size=config.batch_size)
        #train_feeder = DataFeederTacotron2(coord, data_dirs, hparams, config, 16,data_type='train', batch_size=config.batch_size)
        test_feeder = DataFeederTacotron2(coord, data_dirs, hparams, config, 16, data_type='test', batch_size=config.num_test)
...
def main():
    ...
    parser.add_argument('--test_interval', type=int, default=5000)  # 테스트 파일 생성 빈도 5000으로 낮춰서 트레이닝 속도 높이기

    
    #parser.add_argument('--checkpoint_interval', type=int, default=2000) # 2000
    parser.add_argument('--checkpoint_interval', type=int, default=1000) # 1000  # 체크포인트 빈도수 높여서 자주 체크가능하게
